...
/**
 * Same as saveMessage, but tagged source="ai" so it's excluded from the
 * generic history and only surfaces in the !ai conversation.
 */
async function saveAiMessage(psid, role, content) {
  const { error } = await supabase.from("messages").insert({ psid, role, content, source: "ai" });
  if (error) throw error;
}
...
/**
 * Get the last N messages for a user, oldest first, formatted as
...
/**
 * Same as getRecentHistory, but only the !ai-sourced turns (source="ai") —
 * this is what feeds Gemini's context and what !history now displays.
 */
async function getRecentAiHistory(psid, limit = HISTORY_LIMIT) {
  const { data, error } = await supabase
    .from("messages")
    .select("role, content, created_at")
    .eq("psid", psid)
